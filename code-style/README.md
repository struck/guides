...
* Use camelcase or underscores appropriate to the language used, or in the case of an established format, always following suit.
...
* Store 3rd party libraries imported into a project in a vendor/ directory (Example JS lib in a Rails project: app/assets/javascripts/vendor/underscore.js)
* Don't commit to version control modifications made to 3rd party libraries. Write patches instead.
...
* <del>Avoid organizational comments (`# Validations`).</del>
* Avoid ternary operators (`boolean ? true : false`). Use multi-line `if`
  instead to emphasize code branches.
* Prefer `foo: :bar` hash format over `:foo => :bar` format.
...
* Use [heredocs](http://en.wikipedia.org/wiki/Here_document) for multi-line strings.
...
* Use verbose ActiveRecord validation helpers, such as `validates_presence_of :x` instead of `validates :x, presence: true`
...
* Put background job classes in `app/jobs`.
...
* Use [shoulda](https://github.com/thoughtbot/shoulda) in Rails apps.
